...
var tools = {
    ...
    setLog: function (rows) {
        ...
        tools._log = newLog;
        var newScroll = (rows.length - tools.getHeight());
        if ( newScroll < 0 ) {
            newScroll = 0;
        }
        tools._currentScroll = newScroll;
    ...
    redrawLog: function () {
        ...
        for (var i = 0, len = tools.getHeight(); i < len; i++) {
            var thisRow = tools._log[tools._currentScroll + i];
            if ( thisRow && thisRow.Message ) {
                var row = '[' + new Date + '] - ' + thisRow.Message;
                if (row.length > tools._term.width) {
                    var multipleLines = Math.ceil(row.length / tools._term.width);
                    for (var remove = 1; remove < multipleLines; remove++) {
                        len--;
                    }
                }
