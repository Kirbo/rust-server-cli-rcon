...
var client = {
    ...
    Connect: function (config, callbacks) {
		...
		if (newdata.UserId < 70000000000000000 && newdata.Username !== "SERVER") {
    			client._toolsCallbacks['addLogRow']({Message: newdata.Message,Time: data.Time});
		}
            }

            else if (data.Identifier == 1 || data.Identifier == 0) {
		if (!client._toolsCallbacks['dontLogToFile'](data.Message) && config.logfile && config.logfile.length > 0) {
